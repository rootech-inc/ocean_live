class Sales {
    ...
    loadAsset(asset_key) {
        ...
        if(anton.IsRequest(asset)){
            ...
            console.table(asset_msg)

            next = asset_msg['next_car']
    ...
    generateProfoma(model_pk,customer_key=0) {
        // get customers
        let customers = this.salesCustomers('all');
        if(anton.IsRequest(customers)){
            let custs = customers['message']
            let cust_opt = ``
            let defined_customer = false;
            for(let c = 0; c< custs.length;c++){
                let customer = custs[c]
                if(customers.pk == customer_key){
                    cust_opt += `<option value="${customer.pk}" selected>${customer.company}</option>`
                    defined_customer = true
                } else {
                    cust_opt += `<option value="${customer.pk}">${customer.company}</option>`
                }
            ...
            let form = `<input onchange="sales.setProformaPerspective()" id="model" type="hidden" value="${model_pk}">`;
             ...
             if(defined_customer){
                 form += `<label for="customer">CUSTOMER</label><select disabled id="customer" name="customer" class="form-control rounded-0">
                        ${cust_opt}
                    </select>`
             } else {
                 form += `<label for="customer">CUSTOMER</label><select disabled id="customer" name="customer" class="form-control rounded-0">
             ...
             if(customer_key != 0){
                 // get customer
                 let customer = cmms_sales.getCustomer(`${customer_key}`);
                if(anton.IsRequest(customer)){
                    let cust_msg = customer['message'][0]
                    cust_name = cust_msg['name']
                    customer_email = cust_msg['email']
                    customer_phone = cust_msg['mobile'].replace(' ','')
                }
             }

             let autos = {
                 "prospective_name":cust_name,
                 prospective_email:customer_email,
                 prospective_phone:customer_phone,
    ...
    saveProforma() {
        ...
        if(anton.validateInputs(fields)){
            ...
            let err = 0;
            let erm = ""
            if(!$('#prospective_email').val().includes('@')){
                err ++;
