...
class CmmsSales{
    ...
    viewTransactions(deal) {
        ...
        if (response['status_code'] === 200) {
            ...
            if(status === 0){
                ...
                new_tran += fom.button('generate_po','button','success','mr-2',`cmms_sales.deal_2_po('${in_deal['pk']}')`)
            }
    ...
    deal_2_po(deal_pk){
        ...
        try{
            if(confirm("Are you sure?")){
                let deal = this.getDeal(deal_pk);
                if(anton.IsRequest(deal)){
                    let mesg = deal['message']['deal'];
                    let asset = mesg['asset'];
                    let customer = mesg['customer'];
                    console.table(asset)
                    console.table(customer)
                    let asset_key = asset['key'];
                    let cust_key = customer['pk'];

                    sales.generateProfoma(asset_key,cust_key)
                } else {
                    throw deal['message']
                }
            } else {
                kasa.warning("Process Canceled")
            }
    ...
    saveDeal(){
        ...
        // validate values
        let ok = anton.validateInputs(['sales_rep','sales_customer','pur_rep_name','pur_rep_email','pur_rep_phone','asset','stock_type','requirement'])
        if(ok){
            ...
            $('#newDeal').modal('hide')
            kasa.confirm(request['message'],0,'here')
