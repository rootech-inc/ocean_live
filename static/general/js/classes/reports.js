class Reports {
    ...
    * */
    ...
    JobCardsScreen(){
        ...
        if (users['status_code'] === 200){
            ...
            let providers = api.call('VIEW',prov_load,'/appscenter/api/');
            let pr_msg = providers.message;
            let p_opt = ``;
            for (let i = 0;  i < pr_msg.length ; i++) {
                let pp = pr_msg[i];
                p_opt += `<option value="${pp['pk']}">${pp['name']}</option>`
            }
        ...
        <div class="container">
            ...
            <div><label for="provider">Provider</label><select name="provider" id="provider" class="form-control rounded-0">
                <option value="*">All</option>${p_opt}
            </select></div>
        </div>
        `;
    ...
    loadServiceReports(){
        let ids = ['user','status','from','to','provider']
        if(anton.validateInputs(ids)){
            ...
            if(request['status_code'] === 200){
                ...
                let user,from,to,status,provider;
                user = data['user'];
                status = data['status']
                from = data['from']
                to = data['to']
                ...
                let rep_head = `
                    <div class="w-100 d-flex flex-wrap justify-content-between">
                        ...
                        <div class="w-25 d-flex flex-wrap justify-content-end"><button onclick="reports.exportServiceReport('${user}','${from}','${to}','${status}','${provider}')" class="btn btn-info">DOWNLOAD</button></div>
                    </div>
                `
    ...
    exportServiceReport(user,from,to,status,provider){
        let payload  = {
            module:'service_report',
            data:{
                user:user,
    ...
    * */
    ...
    //transfer check
    checkRetailTransferScreen(){
        let form = ``;
        form += fom.text('entry_no','Transfer Number',true)
        form += fom.select('doc',`<option value="json">VIEW</option><option value="excel">EXCEL</option>`,'',true)

        amodal.setBodyHtml(form);
        amodal.setTitleText("CHECK TRANSFER")
        amodal.setFooterHtml(`<button onclick="reports.retailTransferResult()" class="btn btn-success w-100">GENERATE</button>`);
        amodal.show();
    }
    ...
    retailTransferResult() {
        ...
        if(anton.validateInputs(id)){
            ...
            api.v2('VIEW',payload,'/retail/api/').then(response => {
                ...
                if(data['doc'] === 'json'){
                    let message = response['message'];
                    let header = ['BARCODE','ITEM CODE','QUANTITY','NAME','PACKING','PACK QUANTITY','TOTAL UNITS','GRN PACK QUANTITY',"HEALTH"]
                    ...
                    for(let r = 0; r < message.length; r++){
                        ...
                        rows += `
                            <tr class="${bg}">
                                ...
                                <td>${row['name']}</td>
                                <td>${row['packing']}</td>
                                <td class="text-info">${row['um_qty']}</td>
                                <td>${row['total_units']}</td>
                                <td class="text-info">${row['grn_um']}</td>
                                <td>${row['health']}</td>
                    ...
                    this.render(header,rows,`Transfer Analysis for ${data['entry_no']}`);
                    amodal.hide()
                }
            } else {
