class Crm {
    ...
    newLog(){
        ...
        if(anton.IsRequest(sectors)){
            ...
            if(anton.IsRequest(positions)){
                ...
                let city =  `<label class="text-info" for="city">CITY</label><select class="form-control rounded-0" id="city" onchange="crm.loadSuburbForLog()"></select>`
                let area = fom.select('suburb','','',true)

                let region = `<div class="row">
                                        <div class="col-sm-6">${city}</div><div class="col-sm-6">${area}</div>
                                    </div>`
                ...
                let cot = `<option selected disabled value="0">SELECT CITY</option>`
                let cities = api.call('VIEW',cpay,'/adapi/')
                if(anton.IsRequest(cities)){
                    let cts = cities['message'];
                    for (let i = 0; i < cts.length; i++) {
                        let city = cts[i];
                        cot += `<option value="${city['pk']}">${city['name']}</option>`

                    }
                } else {
                    kasa.response(cities)
                }
    ...
    uploadCsvScreen(){
        let all_users = user.allUsers(false)
        if(anton.IsRequest(all_users)){
            let users = all_users['message'];
            let us_option = ``
            for (let index = 0; index < users.length; index++) {
                let user = users[index];
                us_option += `<option value="${user['pk']}">${user['fullname']}</option>`;
            }

            let html = `
            <a href="/static/general/crm-logs-reports/crm_upload_template.csv">Download Template</a><br>
            <select name="" id="upload_owner" class="form-control rounded-0 mb-2">${us_option}</select>
        ...
        if (file) {
          ...
          reader.onload = function (e) {
              ...
              for (let i = 1; i < rows.length; i++) {
                  ...
                  subject = rowData[6];
                  flag = rowData[7];
                  date = rowData[9].replace(/\r/g, '');
                  detail = rowData.slice(8).join(', ');
    ...
    saveNewLog(){
        let fields = ['sector','company_name','contact_person','position','phone','subject','flag','details','mypk','city','suburb','address'];
        if(anton.validateInputs(fields)){
    ...
    extendFollowUpScreen(pk){
        ...
        amodal.setBodyHtml(`
            <p class="text-info">This action will create a new log as of today and adjust reminder date</p>
            <input type="hidden" readonly class="form-control rounded-0 mb-2" value="${pk}" id="f_pk">
            <input type="date" class="form-control rounded-0 mb-2" id="date" >
            <textarea placeholder="reason for extension?" name="" id="new_reason" class="form-control rounded-0 mb-2" cols="30" rows="10"></textarea>
        `)
        amodal.setFooterHtml(`<button onclick="crm.extendFollowUp()" class="btn btn-success my-2">Extend</button>`)
        amodal.show()
    }
    extendFollowUp() {
        let ids = ['date','new_reason','f_pk','mypk']
    ...
    closeFollowUp(pk){
        let payload = {
            module:'close_follow_up',
            data:{
                pk:pk
            }
        }

        let close= api.call('PATCH',payload,'/crm/api/')
        kasa.confirm(close['message'],1,'here')
    }
    ...
    DownloadMyData() {
        ...
        let dl = api.call('VIEW',payload,'/crm/api/')
        if(anton.IsRequest(dl)){
            kasa.html(`<a href="/${dl['message']}">DOWNLOAD</a>`)
        } else {
            kasa.response(dl)
        }
    }
    ...
    createDeal(log_pk) {
        let assets = cmms_sales.getAssetModel('*')
        if(anton.IsRequest(assets)){
            let ass = assets.message;
            let car_opt = ""
            for (let i = 0; i < ass.length; i++) {
                let asset = ass[i];
                let car = asset['car'];
                let model_name = asset['model_name'];
                let year = asset['year'];
                ...
                car_opt += `
                <option value="${pk}">${car_name}  ${model_name} (${year})</option>
                `

            }

            let f = fom.select('interest_in',car_opt,'',true);
            f += fom.textarea('note',5,true)

            amodal.setTitleText("Open Deal")
            amodal.setBodyHtml(f)
            amodal.setFooterHtml(`<button onclick="crm.saveDeal('${log_pk}')" class="btn btn-success w-100">CREATE</button>`)
            $('#note').attr('placeholder',"Any Requirement")
            amodal.show()
    ...
    saveDeal(log_pk) {
        let ids = ['interest_in','note'];
        if(anton.validateInputs(ids)){
            let payload = {
                module:'deal_from_log',
                data:{
                    asset:$('#interest_in').val(),
                    log:log_pk,
                    note:$('#note').val(),
                    mypk:$('#mypk').val(),
                }
            ...
            let generate = api.call('PUT',payload,'/crm/api/');
            if(anton.IsRequest(generate)){
                let message = generate.message;
                location.href = `/cmms/sales/deals/${message}/`
            } else {
                kasa.error(generate)
            }
        } else{
