...
from admin_panel.models import MailQueues, MailSenders, MailAttachments, Sms, SmsApi
...
from servicing.views import jobcard
    ...
    try:
        ...
        if method == 'VIEW':
            try:
                ...
                if module == 'product':
                    ...
                    if SalesCustomers.objects.filter(pk=cust).count() == 1:
                        ...
                        for deal in deals:
                            obj = {
                                ...
                                'asset': {
                                    'name': deal.asset.model_name,
                                    'model': deal.asset.year,
                ...
                elif module == 'open_purchases':
                    purchases = SalesDeals.objects.all()
                    for purchase in purchases:
                        arr.append(purchase.myself())
                    ...
                    print(proforma.car_model.model_name)
                    ...
                    pdf.add_page()
                    pdf.set_font('Arial', 'BU', 10)
                    pdf.cell(180, 5, "JOB CARD", 0, 1, 'C')
                    pdf.ln(10)
                    wd = 180
                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(25,5,f"Customer: ",1,0)
                    pdf.set_font('Arial', '', 10)
                    pdf.cell(50, 5, f"{jobcard.company}", 0, 0)

                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(20, 5, f"Driver: ", 1, 0)
                    pdf.set_font('Arial', '', 10)
                    pdf.cell(35, 5, f"{jobcard.driver}", 0, 0)

                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(25, 5, f"Contact: ", 1, 0)
                    pdf.set_font('Arial', '', 10)
                    pdf.cell(20, 5, f"{jobcard.contact}", 0, 1)

                    pdf.ln(5)

                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(25, 5, f"Brand: ", 1, 0)
                    pdf.set_font('Arial', '', 10)
                    pdf.cell(50, 5, f"{jobcard.brand}", 0, 0)

                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(20, 5, f"Model: ", 1, 0)
                    pdf.set_font('Arial', '', 10)
                    pdf.cell(35, 5, f"{jobcard.model}", 0, 0)

                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(25, 5, f"Number: ", 1, 0)
                    pdf.set_font('Arial', '', 10)
                    pdf.cell(20, 5, f"{jobcard.carno}", 0, 1)



                    pdf.ln(5)
                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(wd, 5, "REPORT ", 0, 1)
                    pdf.set_font('Arial', 'I', 10)
                    pdf.multi_cell(0,5,f"{jobcard.report}",0)
                    pdf.ln(5)


                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(wd / 5,5,"PART NO.",1,0)
                    pdf.cell(wd / 3, 5, "NAME", 1, 0)
                    pdf.cell(wd / 7, 5, "QTY", 1, 0)
                    pdf.cell(wd / 5, 5, "BY", 1, 0)
                    pdf.cell(wd / 5, 5, "Date", 1, 1)

                    pdf.set_font('Arial', '', 10)
                    for material in jobcard.materials():
                        pdf.cell(wd / 5, 5, f"{material.barcode}", 1, 0)
                        pdf.cell(wd / 3, 5, f"{material.part}", 1, 0)
                        pdf.cell(wd / 7, 5, f"{material.quantity}", 1, 0)
                        pdf.cell(wd / 5, 5, f"{material.by()}", 1, 0)
                        pdf.cell(wd / 5, 5, f"{material.issued_on.strftime('%Y-%m-%d')}", 1, 1)

                    pdf.ln(5)
                    pdf.set_font('Arial', 'B', 10)
                    pdf.cell(wd, 5, "Checklist ", 0, 1)
                    images = jobcard.images()

                    # Grid parameters
                    images_per_row = 3  # 3 images per row
                    image_width = 60  # Adjust as per the required width
                    image_height = 60  # Adjust as per the required height
                    margin = 10  # Margin between images and the border

                    # Starting positions
                    x_start = pdf.get_x()  # Get current X
                    y_start = pdf.get_y()  # Get current Y

                    # Loop through images and insert them in a grid
                    for i, image in enumerate(images):
                        img = image.image.path

                        print(img)
                        # Calculate x position (modulus to wrap after 3 images)
                        x_pos = x_start + (i % images_per_row) * (image_width + margin)
                        # Calculate y position (new row after every 3 images)
                        y_pos = y_start + (i // images_per_row) * (image_height + margin)
            ...
            try:
                ...
                if module == 'check_service_due_date':
                    ...
                    mypk = data.get('mypk')
                    entry_date = data.get('save_job_card',timezone.now().date())
                    owner = User.objects.get(pk=mypk)
                    ...
                    response['status_code'] = 200
                    response['message'] = just_saved.pk
                    ...
                    try:
                        ...
                        SalesDeals(customer=customer, owner=owner, pur_rep_name=pur_rep_name,
                                   ...
                                   pur_rep_email=pur_rep_email, asset=asset,
                                   requirement=requirement).save()

                        # save in logs
                        desc = (f"Initiated discussion with {pur_rep_name} from {customer.company} regarding car "
                                f"purchase. Asset presented is"
                                f"{asset.car.name} with model of {asset.model_name}. He has a requirement which is {requirement}")
            ...
            try:
                if module == 'stock':
                ...
                elif module == 'issue_material':
                    pk = data.get('pk')
                    mypk = data.get('mypk')

                    issues_by = User.objects.get(pk=mypk)
                    mat = JobMaterials.objects.get(pk=pk)
                    mat.issues_by = issues_by
                    mat.is_issued = True
                    mat.issued_on = timezone.now()
                    mat.save()

                    response['status_code'] = 200
                    response['message'] = "Material Issued"

                elif module == 'start_job':
                    wr_no = data.get('wr_no')
                    ...
                    carno = job.carno
                    sms = f"""
                    Dear {job.driver}, Servicing for your car with registration {carno} has started with request number {wr_no}\n\nThank You\nSneda Motors Limited
                    """
                    ...
                    JobCardTransactions(
                        ...
                        details=f"""Job card with number {wr_no} has been opened""",
                        owner=owner,
                        jobcard_id=jobpk
                    ).save()
                    ...
                    carno = job.carno
                    sms = f"""
                    Dear {job.driver}, Servicing for your car with registration {carno} has ended, leave us a feedback with link below\nhttps://tinyurl.com/snedamtl\n\nThank You\nSneda Motors Limited
                    """
            ...
            elif module == 'job_card':
                pk = data.get('pk')
                JobCard.objects.filter(pk=pk).delete()
                response['status_code'] = 200
